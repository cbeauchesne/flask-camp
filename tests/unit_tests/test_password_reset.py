...
class Test_PasswordReset(BaseTest):
    def reset_password(self, mail, user):
        with mail.record_messages() as outbox:
            r = self.post("/reset_password", json={"email": user._email}, expected_status=200)
    ...
    def test_email_not_found(self, mail):
        with mail.record_messages() as outbox:
            self.post("/reset_password", json={"email": "i_do@not_exists.fr"}, expected_status=200)
            assert len(outbox) == 0

    def test_user_is_not_validated(self, unvalidated_user, mail):
        with mail.record_messages() as outbox:
            self.post("/reset_password", json={"email": unvalidated_user._email_to_validate}, expected_status=200)
            assert len(outbox) == 0
